...
<body class="bg-slate-900">
    ...
    <script type="text/babel" data-presets="react">
        ...
        const StyleInjector = () => {
            return null; // Styles are in the <head> now
        };
        ...
        const EventModal = ({ isOpen, onClose, onSave, event, selectedDate, selectedTime, members, memberId, presets, notifications }) => {
            ...
            const handleSave = () => {
                ...
                const finalTitle = isCustomTitle ? title : presets.find(p => p.title === title)?.title || title; if (!finalTitle.trim() || !selectedMemberId) { alert("Please enter a title and select a member."); return; } if (timeToMinutes(startTime) >= timeToMinutes(endTime)) { alert("End time must be after start time."); return; } onSave({ id: event && !event.isGoogleEvent ? event.id : null, title: finalTitle, type, status, date: selectedDate.toISOString().split('T')[0], startTime, endTime, memberId: selectedMemberId, color, location, description, notification, recurrence }); onClose(); };
        ...
        function App() {
            ...
            }, [userId, selectedDate.getMonth(), selectedDate.getFullYear(), googleAuth.isGoogleLoggedIn]);
